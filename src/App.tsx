...
function Catalog({disciplines,subjects,sources,types,defaultSourceId,setDefaultSourceId,defaultQuestionTypeId,setDefaultQuestionTypeId,refresh,notify,catalogDeleteOpen,setCatalogDeleteOpen,catalogDeletePassword,setCatalogDeletePassword,catalogDeleteBusy,deleteAllCatalogData}:any) {
  ...
  return <>
    ...
    <section className="section"><div className="section-body">
      ...
      {(kind==="source"||kind==="type")&&<div className="section" style={{marginBottom:"14px",border:"1px solid var(--border, #30343b)"}}>
        <div className="section-body" style={{padding:"14px 16px"}}>
          <strong style={{display:"block",marginBottom:"6px"}}>⚡ Padrão para novos lançamentos</strong>
          <small style={{display:"block",marginBottom:"10px",opacity:.75}}>Escolha um cadastro existente. Ele será preenchido automaticamente no formulário de Novo lançamento.</small>
          <select value={kind==="source"?defaultSourceId:defaultQuestionTypeId} onChange={e=>void setDefaultCatalogItem(kind,e.target.value)} style={{width:"100%"}}>
            <option value="">Nenhum padrão definido</option>
            {list.map((item:any)=><option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
        </div>
      </div>}
