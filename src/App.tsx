...
function SettingsPage({dailyGoal,targetAccuracy,setDailyGoal,setTargetAccuracy,flash,fail}:any){
 const save=async()=>{try{const r=await supabase.from("user_settings").upsert({user_id:await uid(),daily_goal:Math.max(1,Number(dailyGoal)),target_accuracy:Math.min(100,Math.max(0,Number(targetAccuracy)))},{onConflict:"user_id"});if(r.error)throw r.error;flash("Configurações salvas.");}catch(e){fail(e)}};
